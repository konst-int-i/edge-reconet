...
reconet_state_dict = torch.load(
    "saved_models/finetune_20200917163819.pt", map_location=torch.device("cpu")
)
torch_reconet = reconet_torch()
torch_reconet.load_state_dict(state_dict=reconet_state_dict, strict=True)

dummy_input = torch.autograd.Variable(
    torch.rand(1, 3, 216, 512, dtype=torch.float32, device="cpu")
)
...
for name, layer in reconet_state_dict.items():
    if "conv" in name or "inst" in name:
        ...
        if "bias" in name:
            ...
            torch_reco_params[name.replace("bias", "")] = {
                "weight": weight_queue,
                "bias": layer,
            }
            weight_queue = None
