...
def warp_back(image: tf.Tensor, flow: tf.Tensor) -> Tuple[tf.Tensor]:
    ...
    multiplier = tf.constant([2. / width, 2. / height], dtype=tf.float32)
    query_points_on_grid = (query_points_on_grid * multiplier) - 1.0
...
def get_luminance_grayscale(image: tf.Tensor) -> tf.Tensor:
    r_coef = 0.2126
    g_coef = 0.7152
    b_coef = 0.0722
...
def calculate_luminance_mask(optical_flow, previous_image, occlusion_mask):
    flow_grayscale = get_luminance_grayscale(optical_flow)
    previous_im_grayscale = get_luminance_grayscale(previous_image)

    backwards_occlusion_mask = tf.math.abs(previous_im_grayscale - flow_grayscale)
    backwards_occlusion_mask = tf.where(backwards_occlusion_mask < 0.05, 0., 1.)
    backwards_occlusion_mask = tf.expand_dims(backwards_occlusion_mask, axis=-1)
    backwards_occlusion_mask = occlusion_mask - backwards_occlusion_mask
    backwards_occlusion_mask = tf.where(backwards_occlusion_mask < 0, 0., 1.)
    return backwards_occlusion_mask
