...
class ReCoNetLoss:
    ...
    _content_loss:           float = 0.
    _style_loss:             float = 0.
    _feature_temporal_loss:  float = 0.
    ...
    def __call__(self, current_vgg_out, current_vgg_in, previous_vgg_out,
        ...
        style_loss_ = self.beta * style_loss(current_vgg_out, style_gram_matrices) \
                    + self.beta * style_loss(previous_vgg_out, style_gram_matrices)

        total_variation_ = self.gamma * total_variation(current_output_frame) \
                         + self.gamma * total_variation(previous_output_frame)

        feature_temporal_loss_ = self.lambda_f * feature_temporal_loss(
                current_feature_maps, previous_feature_maps,
                reverse_optical_flow, occlusion_mask)

        output_temporal_loss_ = self.lambda_o * output_temporal_loss(
                current_input_frame, previous_input_frame,
                current_output_frame, previous_output_frame,
                reverse_optical_flow, occlusion_mask)
...
def content_loss(content_feature_maps, style_feature_maps):
    ...
    return tf.reduce_sum(tf.square(content_feature_maps - style_feature_maps)) / (c * w * h)


def style_loss(content_feature_maps, style_gram_matrices):
    return tf.reduce_sum((tf.square(gram_matrix(content) - style))
            for content, style in zip(content_feature_maps, style_gram_matrices))


def feature_temporal_loss(current_feature_maps, previous_feature_maps,
        reverse_optical_flow, occlusion_mask):
    b, w, h, c = current_feature_maps.shape
    feature_maps_diff = current_feature_maps - warp_back(previous_feature_maps, reverse_optical_flow)
    loss = tf.reduce_sum(tf.square(occlusion_mask * feature_maps_diff)) / (c * h * w)


def output_temporal_loss(current_input_frame, previous_input_frame,
        current_output_frame, previous_output_frame, reverse_optical_flow,
        occlusion_mask):
    input_diff = current_input_frame - warp_back(previous_input_frame, reverse_optical_flow)
    output_diff = current_output_frame - warp_back(previous_output_frame, reverse_optical_flow)
    luminance_input_diff = tf.expand_dims(get_luminance_grayscale(input_diff))

    b, w, h, c = current_input_frame.shape
    loss = tf.reduce_sum(tf.square((occlusion_mask * (output_diff - luminance_input_diff)))) / (h * w)
    return loss


def total_variation(im):
    return tf.reduce_sum(tf.math.abs(im[:, :, :-1, :] - im[:, :, 1:, :])) + \
