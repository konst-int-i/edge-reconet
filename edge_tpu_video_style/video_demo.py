...
def min_max_scale_tf(tensor):
    return (tensor - tf.reduce_min(tensor, keepdims=True)) / (tf.reduce_max(tensor, keepdims=True) - tf.reduce_min(tensor, keepdims=True))
...
def show_webcam(model, mirror=False, input_location='/dev/video0',
    ...
    if output_location != 'display':
        out = cv2.VideoWriter(output_location,
                cv2.VideoWriter_fourcc('M','J','P','G'), 24, output_size)
    while True:
        ...
        cv2_im_rgb = cv2.resize(img, inference_size)
        inp = cv2_im_rgb
